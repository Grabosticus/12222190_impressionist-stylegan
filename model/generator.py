        ...
        for resolution_key in sorted(self.styled_convs.keys(), key=lambda k: int(k)):
            ...
            if int(resolution_key) == self.current_res:
                ...
                if self.layer_opacity.item() < 1.0:
                    prev_rgb_upsampled = F.interpolate(rgb, scale_factor=2, mode=globals.INTERPOLATION_MODE, align_corners=False)
                    output = linear_interpolation(prev_rgb_upsampled, new_rgb, self.layer_opacity.item())
                else:

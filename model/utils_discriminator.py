...
class MiniBatchStandardDeviation(nn.Module):
    ...
    def __init__(self, group_size=4):
        super().__init__()
        self.group_size = (
            group_size  # the size of sub-batches we compute the standard deviation over
        )
    ...
    def forward(self, x):
        ...
        groups = x.view(
            actual_group_size, -1, n_channels, height, width
        )  # we reshape the tensor to reflect the group size
        variance = (
            torch.var(groups, dim=0, unbiased=False) + 1e-8
        )  # the variance for pixels each group
        stddev = torch.sqrt(variance)
        means_of_stddev = stddev.mean(
            dim=[1, 2, 3], keepdim=True
        )  # the actual average stddev for this group (a single value)
        means_of_stddev = means_of_stddev.repeat(
            actual_group_size, 1, height, width
        )  # we just resize the average stddev to match our input shape
        return torch.cat([x, means_of_stddev], dim=1)


def d_loss_non_saturating_r1(
    D, real_imgs, fake_imgs, d_step, ada: ADA, gamma=10, log=False
):
    """

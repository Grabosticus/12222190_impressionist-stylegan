...
class AdaIN(nn.Module):
    ...
    def forward(self, x, w):
        ...
        std = x.var(dim=(2,3), unbiased=False, keepdim=True).add(1e-8).sqrt() # stddev over all pixels in each channel
        x = (x - mean) / std
        return ((1+y_s) * x) + y_b
...
class StyledConv(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, w_dim=globals.W_DIM, kernel_size=globals.KERNEL_SIZE, upsample=False):
        super().__init__()
    ...
    def forward(self, x, w, noise=None):
        if self.upsample:
            x = F.interpolate(x, scale_factor=2, mode=globals.INTERPOLATION_MODE, align_corners=False)
        x = self.conv(x)
